...
class SpectralBeatMainView extends Component {
	constructor(props) {
		super(props);
		this.jumpNumber = 0;
		this.videoFileName = 'videos/grabowsee480.mp4';
	...
	jumpToRandomTimestamp() {
		const i = this.jumpNumber % NUM_PLAYERS;
		...
		return (
			<div style={{ backgroundColor, ...styles.main }}>
				<div style={styles.fullscreenButtonWrapper}>
					<img src={'fullscreen.png'} style={styles.fullscreenButton} onClick={toggleFullScreen} />
				</div>
				<div style={styles.videoContainer} ref={this.videoContainers[0]}>
					<ReactPlayer ref={this.players[0]} url={this.videoFileName} width="100%" height="100%" playing muted loop onClick={() => this.jumpToRandomTimestamp()} />
				</div>

				<div style={styles.videoContainer} ref={this.videoContainers[1]}>
					<ReactPlayer ref={this.players[1]} url={this.videoFileName} width="100%" height="100%" playing muted loop onClick={() => this.jumpToRandomTimestamp()} />
				</div>
				<div style={styles.videoContainer} ref={this.videoContainers[2]}>
					<ReactPlayer ref={this.players[2]} url={this.videoFileName} width="100%" height="100%" playing muted loop onClick={() => this.jumpToRandomTimestamp()} />
				</div>

				<div style={styles.settingsContainer}>{settingsAreVisible ? <SettingsOverlay /> : null}</div>
...
export default connect(mapStateToProps, mapDispatchToProps)(SpectralBeatMainView);
